...
void senalOut() {                         //Funcion que se encarga de generar los estados de los semiciclos correspondientes.
...
void acelerar() {
  for (i = 1; i < countMax; i++) {        //se calculo que aproximadamente los 928hz se alcanzan los 100km/h
    senalOut();
    velMax();
    lecturasEnSubida05();
    lecturasEnSubida6();
...
void error_001() { //error 1 cuando X1_VT_05 debiera ser HIGH
  Serial.println("");
  for (;;) {
...
void error_002() { //error 2 cuando X1_VT_6 deberia ser HIGH
  for (;;) {
    for (j = 0; j < 2; j++) {
      digitalWrite(error, HIGH);
      delay(500);
...
void error_003() { //error 3 cuando X7_VT_05 deberia ser HIGH
  for (;;) {
    for (j = 0; j < 3; j++) {
      digitalWrite(error, HIGH);
      delay(500);
...
void error_004() { //error 4 cuando X7_VT_6 deberia ser HIGH
  for (;;) {
    for (j = 0; j < 4; j++) {
      digitalWrite(error, HIGH);
      delay(500);
...
void error_005() { //error 5 cuando X6_VT_05 deberia ser HIGH
  for (;;) {
    for (j = 0; j < 5; j++) {
      digitalWrite(error, HIGH);
      delay(500);
...
void error_006() { //error 6 cuando X6_VT_6 deberia ser HIGH
  for (;;) {
    for (j = 0; j < 6; j++) {
      digitalWrite(error, HIGH);
      delay(500);
...
void error_007() { //error 7 Ninguna lectura cambia
  for (;;) {
    for (j = 0; j = 7; j++) {
      digitalWrite(error, HIGH);
      delay(500);
...
void lecturasEnSubida05() {
  ...
  while (vel > vel_1) {                              //Cuando la velocidad calculada es MAYOR a 0.5KM/h y...
    if (digitalRead(X1_VT_05) == LOW ) {             //Si la lectura es 0 digital, se ejecuta el error 1
    ...
    else if (digitalRead(X7_VT_05) == LOW ) {        //Si la lectura es 0 digital, se ejecuta el error 3
      Serial.println("ERROR_021: la variable X7_VT_05 debia arrojar un UNO, pero recibio un CERO\nERROR CONTROL DE VELCIDAD RELE 0.5KM/H");
      error_003();
      break;
    }
    else if (digitalRead(X6_VT_05) == LOW ) {         //Si la lectura es 0 digital, se ejecuta el error 5
...
void lecturasEnSubida6(){
  ...
  while (vel > vel_2) {                                //Cuando la velocidad calculada es MENOR a 6km/h y...
    if (digitalRead(X1_VT_6) == LOW) {                 //Si la lectura es 1 digital, se ejecuta el error 2
    ...
    else if (digitalRead(X7_VT_6) == LOW) {             //Si la lectura es 1 digital, se ejecuta el error 4
      Serial.println("ERROR_022: la variable X7_VT_6 debia arrojar un UNO, pero recibio un CERO\nERROR CONTROL DE VELCIDAD RELE 6KM/H");
      error_004();
      break;
    }
    else if (digitalRead(X6_VT_6) == LOW) {             //Si la lectura es 1 digital, se ejecuta el error 6
...
void lecturasEnBajada05() {
  ...
  Serial.println(digitalRead(X7_VT_05));
  Serial.println(digitalRead(X6_VT_05));
  while (vel <= vel_1) {                                  //Si la velocidad calculada es MENOR a 0.5KM/h y...
    if (digitalRead(X1_VT_05) == HIGH ) {                 //Si la lectura es 1 digital, se ejecuta el error 1
      Serial.println("ERROR_013: la variable X1_VT_05 debia arrojar un CERO, pero recibio un UNO\nERROR CONTROL DE VELCIDAD RELE 0.5KM/H");
      error_001();
      break;
    }
    else if (digitalRead(X7_VT_05) == HIGH ) {            //Si la lectura es 1 digital, se ejecuta el error 3
      Serial.println("ERROR_023: la variable X7_VT_05 debia arrojar un CERO, pero recibio un UNO\nERROR CONTROL DE VELCIDAD RELE 0.5KM/H");
      error_003();
      break;
    }
    else if (digitalRead(X6_VT_05) == HIGH ) {             //Si la lectura es 1 digital, se ejecuta el error 5
...
void lecturasEnBajada6(){
  ...
  Serial.println(digitalRead(X7_VT_6));
  Serial.println(digitalRead(X6_VT_6));
  while (vel <= vel_2) {                                    //Si la velocidad calculada es MENOR a 6km/h y...
    if (digitalRead(X1_VT_6) == HIGH) {                     //Si la lectura es 1 digital, se ejecuta el error 2
      Serial.println("ERROR_014: la variable X1_VT_6 debia arrojar un CERO, pero recibio un UNO\nERROR CONTROL DE VELCIDAD RELE 6KM/H");
      error_002();
      break;
    }
    else if (digitalRead(X7_VT_6) == HIGH) {                //Si la lectura es 1 digital, se ejecuta el error 2
      Serial.println("ERROR_024: la variable X7_VT_6 debia arrojar un CERO, pero recibio un UNO\nERROR CONTROL DE VELCIDAD RELE 6KM/H");
      error_004();
      break;
    }
    else if (digitalRead(X6_VT_6) == HIGH) {                //Si la lectura es 1 digital, se ejecuta el error 2
